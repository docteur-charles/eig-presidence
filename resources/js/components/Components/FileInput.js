...
export default function FileInput({ _id = 0, onLoaded }) {
  ...
  function onBlur(e) {
    ...
    if (file) {
      window.$(e.currentTarget).find('b').css({ color: '#000' });
    }
  }
  ...
  function onFocus(e) {
    if (!file) {
      ...
      window.$(e.currentTarget).find('b').css({ color: '#ffa500f0' });;
    }
  }
        ...
        <label style={{
          ...
          {file ? (
            <>
              ...
              <span className="align-self-center">PDF, PNG, JPG, JPEG, BITMAP</span>
            )}
        </label>
        <label style={{
          cursor: 'pointer',
      ...
      {!file && (
        <>
          ...
          <small className="form-text d-none d-md-inline text-center pr-2 pl-2 text-muted">
            Cliquez dans la zone de texte et chargez le courrier à enregistrer depuis un espace de stockage.
          </small>
        </>
      )}
    </div>
