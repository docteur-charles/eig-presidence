...
import { REGISTER_CONFIDENTIALS } from "../Helpers/Const";
import { useMutation } from "react-query";
import CourrierService from "../Services/Courrier";
import { useSession } from "../Context/Session";

export default function Enregistrement() {
    const { mutateAsync: registerIncoming, isLoading, isSuccess } = useMutation(
        CourrierService.RegisterIncoming
    );
    ...
    const handleSubmit = React.useCallback(
        async e => {
            ...
            try {
                ...
                const { message, courrier } = await registerIncoming({
                    data: formData
                });
                swal("Enregistrement terminé", message, "success").then(() => {
                    swal.close();
                    history.push(`/courriers/${courrier}`);
                });
            } catch (err) {
                console.log(err);
            }
                            ...
                            <div className="row">
                                <div className="form-group col-12 col-md-6">
                                    ...
                                    >
                                        ...
                                        {user.role.privileges.some(privilege =>
                                            REGISTER_CONFIDENTIALS.includes(
                                                privilege
                                            )
                                        ) && (
                                ...
                                <div className="form-group col-12 col-md-6">
                                    ...
                                    >
                                        ...
                                        <option disabled value="SORTANT">
                                            Sortant
                                        </option>
                                        <option disabled value="INTERNE">
                                            Interne
                                        </option>
                                    </select>
                                </div>
